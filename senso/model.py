...
class UNet(nn.Module):
    ...
    def __init__(self, im_height, im_weight, in_ch=1, out_ch=3, dropout_rate=0.1) -> None:
        """
        Args:
           ...
           dropout_rate: Dropout normalisation. 
        Returns:
            
        """
        super(UNet, self).__init__()
        self.encode_A = ConvConv(1, 64)
        ...
        self.upsample_A = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
            nn.Dropout(dropout_rate),
            nn.Conv2d(1024, 512, kernel_size=2, padding=1, stride=1)
        )
        self.decode_A = ConvConv(1024, 512)
        self.upsample_B = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
            nn.Dropout(dropout_rate),
            nn.Conv2d(512, 256, kernel_size=2, padding=1, stride=1)
        )
        self.decode_B = ConvConv(512, 256)
        self.upsample_C = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
            nn.Dropout(dropout_rate),
            nn.Conv2d(256, 128, kernel_size=2, padding=1, stride=1)
        )
        self.decode_C = ConvConv(256, 128)
        self.upsample_D = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
            nn.Dropout(dropout_rate),
            nn.Conv2d(128, 64, kernel_size=2, padding=1, stride=1)
        )
    ...
    def forward(self, x):
        ...
        x = torch.cat([down_D, x], dim=1) #TO DO: Check if dim is correct.
        x = self.decode_A(x)
        x = self.upsample_B(x)
